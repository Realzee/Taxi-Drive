...
// === EDIT (Added basic edit functions for completeness) ===
async function editDriver(id) {
  ...
  // Override save button to update instead
  const saveBtn = document.querySelector('#modal-content button[onclick="saveDriver()"]');
  ...
  saveBtn.onclick = async () => {
    ...
    await supabase.from('drivers').update(updatedDriver).eq('id', id);
    closeModal();
    loadDrivers();
  };
}
...
async function editVehicle(id) {
  ...
  // Override save button to update instead
  const saveBtn = document.querySelector('#modal-content button[onclick="saveVehicle()"]');
  saveBtn.textContent = 'Update';
  saveBtn.onclick = async () => {
    ...
    await supabase.from('vehicles').update(updatedVehicle).eq('id', id);
    closeModal();
    loadVehicles();
  };
}
...
// Additional function for logout (assuming it's needed)
